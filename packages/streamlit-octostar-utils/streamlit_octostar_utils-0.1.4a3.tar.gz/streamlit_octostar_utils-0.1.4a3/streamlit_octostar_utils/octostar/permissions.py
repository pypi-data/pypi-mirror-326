...
def get_workspaces(client, prev_workspaces=None, include_closed=False):
    if not include_closed:
        ...
        workspace_ids = [w['os_entity_uid'] for w in client.execute(query_ontology.sync, f"SELECT `os_entity_uid` FROM `dtimbr`.`os_workspace`")]
    ...
    workspaces = client.execute(query_ontology.sync, f"SELECT `entity_label`, `os_entity_uid` FROM `dtimbr`.`os_workspace` WHERE `os_entity_uid` IN ({workspace_ids})")
    workspaces_permissions = client.execute(get_permissions.sync, [w['os_entity_uid'] for w in workspaces])
    ...
    return dict(filter(lambda x: x[1]['os_permission'] > PermissionLevel.NONE, workspaces.items()))