...
class CombineFieldLast(CombineField):
    def __init__(self, field, primitive_types_only=True, **params):
        super().__init__(field, **{**params, **{'primitive_types_only': primitive_types_only}})
    
    def combine_field(self, data, entity):
        primitive_types_only = False
        if 'primitive_types_only' in self.params and self.params['primitive_types_only']:
        ...
        if data:
            ...
            if primitive_types_only and isinstance(last_elem, (dict, list)):
                if len(last_elem) == 1:
                    ...
                    raise ValueError("Primitive type only are allowed in output for field " + self.field)
            if last_elem:
                return last_elem
            else:
                return ''
...
class CombineFieldConcat(CombineField):
    def concat(self, data, dict_sep='', list_sep='', as_flat_list=False):
        ...
        if as_flat_list:
            ...
            return dict_sep.join([list_sep.join([elem for elem in ls]) for ls in data.values()])
...
class CombineFieldSpawn(CombineField):
    def __init__(self, field, inner_combiner=None, **params):
        ...
        if not self.inner_combiner:
            self.inner_combiner = CombineFieldCustom(field, lambda d, e: d) # Special combiner that should return a list or dict of lists
    def combine_field(self, data, entity):
        def _list_flatten(in_list):
            in_list = list(in_list)
            if in_list == []:
        ...
        value_dicts = dict()
        all_params = list()
        entity_relationships = ([rel for rel in self.parser.curr_relationships if rel.concept_from == entity.unique_id],
                                [rel for rel in self.parser.curr_relationships if rel.concept_to == entity.unique_id])
        for field_key, field_value in entity.fields.items():
            if isinstance(field_value, CombineFieldInfo) and isinstance(field_value.combiner, CombineFieldSpawn):
                values = field_value.combiner.inner_combiner.combine_field(field_value.data, entity)
        ...
        all_params = {param: max([len(inner_dict.get(param, [])) for inner_dict in value_dicts.values()]) for param in all_params}
        value_lists = dict()
        for field_key, inner_dicts in value_dicts.items():
            ...
            for param_name in inner_dicts.keys():
                inner_dicts[param_name].extend([None]*(all_params[param_name]-len(inner_dicts[param_name])))
        keys = list(value_dicts.keys())
        value_lists = [_list_flatten([inner_dict[key] for key in sorted(list(inner_dict.keys()))]) for inner_dict in value_dicts.values()]
        if value_lists:
            maxlen = max([len(ls) for ls in value_lists])
            [ls.extend([None]*(maxlen-len(ls))) for ls in value_lists]
            combined_fields = [{keys[i]: value_lists[i][j] for i in range(len(keys))} for j in range(maxlen)]
            ...
            for combined_fields_elem in combined_fields:
                ...
                new_entity = EntityInfo(str(uuid.uuid4()), entity.concept_name, new_fields, parents=entity.parents)
                new_rels = [RelationshipInfo(new_entity.unique_id, rel.concept_to, rel.name) for rel in entity_relationships[0]] +\
                        [RelationshipInfo(rel.concept_from, new_entity.unique_id, rel.name) for rel in entity_relationships[1]]
