    ...
    def _worker_thread(self):
        while not self.kill_event.is_set():
            ...
            try:
                ...
                result = task(AsyncTaskManager.TaskData(self.output, task_id, canc_event),
                                **task_kwargs)
                self.output(task_id, AsyncTaskManager.TaskStatus.SUCCESS, result)
            except AsyncTaskManager.CancelledError:
                self.output(task_id, AsyncTaskManager.TaskStatus.CANCELLED, self.get_result(task_id)['data'])
    ...
    def cancel(self, task_id):
        ...
        try:
            ...
            try:
                ...
                pass
        if cancelled:
            self.output(task_id, AsyncTaskManager.TaskStatus.CANCELLING, self.get_result(task_id)['data'])
        return cancelled

    def cancel_all(self):
        all_task_ids = set(self.tasks_queue.list_keys() + self.cancellation_tokens.list_keys())
