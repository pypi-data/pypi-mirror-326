        ...
        for file_name in os.listdir(in_dir):
            ...
            if not redis_client.exists(redis_key):
                ...
                if os.path.exists(os.path.join(out_dir, file_name)):
                    stale = (time.time() - os.path.getmtime(file_path) > result_expires)
                else:
                    stale = (time.time() - os.path.getmtime(file_path) > task_expires)
                if stale:
                    try:
    ...
    def set_cleanup_task(self):
        ...
        self.app.conf.beat_schedule = {
            'celery.backend_cleanup': {
                ...
                'args': (str(self.in_folder), str(self.out_folder),
                         self.redis_host, self.redis_port,
                         self.app.conf.task_expires, self.app.conf.result_expires)
            },
        }
