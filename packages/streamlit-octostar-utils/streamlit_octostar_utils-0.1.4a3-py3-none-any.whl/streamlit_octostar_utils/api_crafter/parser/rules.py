...
class RegexRule(Rule):
    def __init__(self, regex, only_primitive_values=False, only_selected_values=None, **params):
        super().__init__(**{**{'only_primitive_values': only_primitive_values}, **params})
    ...
    def apply_rule(self, field, value):
        return self.type_rule.apply_rule(field, value) and (self.regex.search(field) is not None)
...
class BoolRule(Rule):
    ...
    def unique_hex(self):
        return hashlib.md5((self.salt + str(self.condition)).encode("utf-8")).hexdigest()
    def apply_rule(self, field, value):
...
class TypeRule(Rule):
    ...
    def unique_hex(self):
        return hashlib.md5((self.salt + str(self.accepted_types)).encode("utf-8")).hexdigest()
    def apply_rule(self, field, value):
...
class AndRule(Rule):
    ...
    def unique_hex(self):
        return hashlib.md5((self.salt + "".join([str(rule.__hash__()) for rule in self.rules])).encode("utf-8")).hexdigest()
    def apply_rule(self, field, value):
        is_true = True
