...
@dataclass
class SinterMWPFRationalDecoder:
    """
...
@dataclass
class SinterHUFRationalDecoder(SinterMWPFRationalDecoder):
    decoder_type: str = "SolverSerialUnionFind"
    cluster_node_limit: int = 0


@dataclass
class SinterSingleHairRationalDecoder(SinterMWPFRationalDecoder):
    decoder_type: str = "SolverSerialSingleHair"
    cluster_node_limit: int = 0


class MwpfRationalCompiledDecoder:
    def __init__(
        self,
...
def detector_error_model_to_mwpf_solver_and_fault_masks(
    ...
    if decoder_type is None:
        ...
        decoder_cls = getattr(mwpf_rational, decoder_type)
    else:
