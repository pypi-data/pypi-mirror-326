...
class REPLICA(object):
    ...
    @property
    def ncnv(self):
        nv=N.count_nonzero(N.array(self.pat==0,dtype=N.int64))
        nc=len(self.pat)
        return(nc,nv)
...
class PS(object):
    ...
    def parse_query(self,cursor):
        ...
        xmin   = N.zeros((self.nvar,),dtype=N.float64)
        xmax   = N.zeros((self.nvar,),dtype=N.float64)
        ymin   = N.zeros((self.nvar,),dtype=N.float64)
        ymax   = N.zeros((self.nvar,),dtype=N.float64)
        gain   = N.zeros((self.nvar,),dtype=N.float64)
        offset = N.zeros((self.nvar,),dtype=N.float64)
        
        sco    = N.zeros((self.nvar,),dtype=N.float64) #offset traditional unscaling of Rostta output
        scs    = N.zeros((self.nvar,),dtype=N.float64) #slope traditional unscaling of Rostta output
        sct    = N.zeros((self.nvar,),dtype=N.int64) # 0: no transform, 1: log10 transform

        d_min  = N.zeros((self.nvar,),dtype=N.float64)
        d_max  = N.zeros((self.nvar,),dtype=N.float64)
    ...
    def check_data(self,data_in):
        ...
        if ssc_set.issubset(vnset):
            ...
            ssc_sum=N.zeros((nsamp,),dtype=N.float64)
...
#        print(self.model_name)
    ...
    def predict(self, data):
        ...
        res_tmp=N.zeros((self.nmodel, self.nout, nsamp_valid),dtype=N.float64)
        ...
        if nsamp==nsamp_valid:
            ...
            res_fin=N.ones((self.nmodel,self.nout,nsamp),dtype=N.float64)*-9.9
...
class PTF_MODEL(object):
    ...
    def sum_stat(self,res,nvar,nsamp):
        ...
        std  = N.zeros((nvar,nsamp),dtype=N.float64)
        skew = N.zeros((nvar,nsamp),dtype=N.float64)
        kurt = N.zeros((nvar,nsamp),dtype=N.float64)
        #print(N.shape(res))
        cov=N.zeros((nvar,nvar,nsamp),dtype=N.float64)
    ...
    def predict(self,data_in,sum_data=True): 
        ...
        data=N.array(data_in,dtype=N.float64)
        ...
        if sum_data:
            ...
            sum_res_mean=N.zeros((nout_total,nsamp),dtype=N.float64)
            sum_res_std=N.zeros((nout_total,nsamp),dtype=N.float64)
            sum_res_skew=N.zeros((nout_total,nsamp),dtype=N.float64)
            sum_res_kurt=N.zeros((nout_total,nsamp),dtype=N.float64)
            sum_res_cov=N.zeros((nout_total,nout_total,nsamp),dtype=N.float64)
