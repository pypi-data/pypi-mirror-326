...
class BlobProxy(FuturizedServiceProxy):
    ...
    def create_blob(self, path: str, blob: object):
        ...
        elif (isinstance(blob, dict | set | list | tuple | int | float | str) or
              (hasattr(blob, "to_csv") and callable(blob.to_csv)) or
              (hasattr(blob, "dtype") and hasattr(blob, "shape") and callable(blob.dtype) and callable(blob.shape))):
            # basic type or pandas dataframe or numpy array
            filedata = pickle.dumps(blob)
        else:
            raise BQApiError("Unable to determine type of %s: must be basic type, numpy array, dataframe, or file", blob)
