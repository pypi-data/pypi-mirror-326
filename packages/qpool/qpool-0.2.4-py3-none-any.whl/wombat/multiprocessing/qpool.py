...
class Worker:
    ...
    async def run(self, actions: Dict[str, Callable], props: Dict[str, Prop]):
        ...
        for prop_name, prop in props.items():
            ...
            resolved_value = prop.instance

            # Resolve value if it hasn't been initialized
            ...
            if prop.use_context_manager:
                if hasattr(resolved_value, "__aenter__") and hasattr(resolved_value, "__aexit__"):
                    resolved_value = await exit_stack.enter_async_context(resolved_value)
                elif hasattr(resolved_value, "__enter__") and hasattr(resolved_value, "__exit__"):
