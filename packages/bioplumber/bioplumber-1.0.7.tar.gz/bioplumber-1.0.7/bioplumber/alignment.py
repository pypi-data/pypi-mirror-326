...
def mmseqs_download_database_(
    ...
    **kwargs
    ...
    if container=="none":
        command = f"mmseqs search {database_name} {output_dir} {output_dir_parent}/tmp"
        for key,value in kwargs.items():
            command = command + f" --{key} {value}"
    
    elif container=="docker":
        command = f"docker run -v {output_dir_parent}:{output_dir_parent} {config.docker_container} mmseqs search {database_name} {output_dir} {output_dir_parent}/tmp"
        for key,value in kwargs.items():
            command = command + f" --{key} {value}"
    
    elif container=="singularity":
        command = f"singularity exec --bind {output_dir_parent}:{output_dir_parent} {config.singularity_container} mmseqs search {database_name} {output_dir} {output_dir_parent}/tmp"
        for key,value in kwargs.items():
            command = command + f" --{key} {value}"
