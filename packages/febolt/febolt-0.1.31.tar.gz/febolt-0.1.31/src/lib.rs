...
fn fit_ols_f32(
    ...
    let mut xtw = Array2::<f32>::zeros((n, k)); // Changed from (k, n) to (n, k)
    for i in 0..n {
        let wi = w[i];
        let row = exog.row(i);
        for j in 0..k {
            xtw[(i, j)] = row[j] * wi;  // Corrected index order
        }
    }
    let xtwx = exog.t().dot(&xtw);  // Now valid (k x n) * (n x k) = (k x k)
    let xtwy = exog.t().dot(&(endog * &w));  // Unchanged
    let beta = xtwx
        .inv()
...
fn fit_ols_f64(
    ...
    let mut xtw = Array2::<f64>::zeros((n, k)); // Changed from (k, n) to (n, k)
    for i in 0..n {
        let wi = w[i];
        let row = exog.row(i);
        for j in 0..k {
...
fn fit_logit<'py>(
    ...
    if !use_f64 {
        ...
        let se = cov_final.diag().mapv(|v| v.sqrt());
        let z = params.clone() / &se;
        let p = z.mapv(|z| 2.0 * (1.0 - probit_f32::cdf32(z.abs())));
        let pd = py.import("pandas")?;
        let data = PyDict::new(py);
        data.set_item("Coefficients", params.to_vec())?;
        data.set_item("Std. Err", se.to_vec())?;
        data.set_item("z", z.to_vec())?;
        data.set_item("Pr(>|z|)", p.to_vec())?;
...
fn fit_ols<'py>(
    ...
    if !use_f64 {
        ...
        let pd = py.import("pandas")?;
        let data = PyDict::new(py);
        data.set_item("Coefficients", beta.to_vec())?;
        data.set_item("Std. Err", se.to_vec())?;
        data.set_item("z", z.to_vec())?;
        data.set_item("Pr(>|z|)", p.to_vec())?;
        let kwargs = PyDict::new(py);
...
fn ame_f32<'py>(py: Python<'py>, model: &PyAny) -> PyResult<&'py PyAny> {
    ...
    // Filter out intercept terms from variable names
    let filtered_names: Vec<String> = exog_names.iter()
        .enumerate()
        .filter(|(j, _)| !intercept_indices.contains(j))
    ...
    let kwargs = PyDict::new(py);
    kwargs.set_item("data", data)?;
    kwargs.set_item("index", filtered_names)?;  // Use filtered names instead of original
