...
class ClientsAPI:
    ...
    def get(self, identificador_tipo: str, identificador_valor: str) -> Dict[str, Any]:
        try:    
            response = requests.get(
                f"{self.api.base_url}/api/external/clients/getclient?identificador_tipo={identificador_tipo}&identificador_valor={identificador_valor}",
                headers=self.api.get_headers()
            )    
            return response.json()
        except Exception as error:
