...
class ClientsAPI:
    ...
    def delete(self, data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            response = requests.delete(
                f"{self.api.base_url}/api/external/clients/delete?identificador_tipo={data['identificador_tipo']}&identificador_valor={data['identificador_valor']}",
                headers=self.api.get_headers()
            )
            return response.json()
        except Exception as error:
    ...
    def get(self, indentificador_tipo: str, identificador_valor: str) -> Dict[str, Any]:
        try:    
            response = requests.get(
                f"{self.api.base_url}/api/external/clients/getclient?identificador_tipo={indentificador_tipo}&identificador_valor={identificador_valor}",
                headers=self.api.get_headers()
            )    
            return response.json()
        except Exception as error:
