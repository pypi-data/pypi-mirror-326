...
class TransformationCommand(SupportsCliCommand):
    ...
    help_string = "Run transformations dlt+ project. Experimental."
    description = """
...
"""
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        if args.operation == "list":
            # _import_ponds()
            available_transformations = list(run_context.config.transformations)
            fmt.echo(f"Available transformations: {available_transformations}")

        # individual pond ops
        transformation = factory.create_transformation(args.pond_name)
