...
class Transformation:
    ...
    def populate_state(self) -> None:
        ...
        # load states
        process_loads_relation = self.state_output_pipeline.dataset()[PROCESSED_LOAD_IDS_TABLE]
        cache_pipeline = self.cache.get_cache_pipeline(True)
        cache_pipeline.run(
    ...
    def flush_state(self) -> None:
        ...
        if not self.state_output_pipeline:
            return
        cache_output_pipelien = self.cache.get_cache_pipeline(True)
        self.state_output_pipeline.run(
            cache_output_pipelien.dataset()[PROCESSED_LOAD_IDS_TABLE].iter_arrow(50000),
            table_name=PROCESSED_LOAD_IDS_TABLE,
            write_disposition="replace",
        )
    ...
    def transform(self) -> None:
        ...
        with self.cache.get_cache_pipeline(transformed_data=True).destination_client() as client:
    ...
    def transformation_layer_path(self) -> str:
        # make it abs path
        return os.path.join(self.ponds_path, self.config["package_name"])
    ...
    def ponds_path(self) -> str:
        ...
        explicit_path = self.config.get("location")
        if explicit_path:
            return os.path.abspath(explicit_path)
        # TODO: never take current context in pond. bind pond to context
        # init (when it is created)
        return dlt.current.run_context().get_run_entity("transformations")
