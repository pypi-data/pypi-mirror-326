...
from databricks.sdk.service.compute import ClusterDetails, Language
...
def validate_data_prep(data_prep_cluster: Optional[str] = None):
    ...
    try:
        ...
        # Creating the context should error for clusters without EXECUTE permissions
        print("Validating that the cluster you provided has execute permissions...")
        context = w.command_execution.create(cluster_id=data_prep_cluster, language=Language.PYTHON).result()
        # To be safe, run a simple command on the cluster
        print("Executing '1+1' on the cluster you provided...")
        result = w.command_execution.execute(cluster_id=data_prep_cluster,
                                             command="1+1",
                                             context_id=context.id,
                                             language=Language.PYTHON)
        test_execution = result.result().results.data == '2'  # FYI this takes ~18s to execute
        print("Completed execution check!")

        if not test_execution:
            raise ValidationError(
                f'You do not have access to execute commands on the cluster you have provided: {data_prep_cluster}. '
