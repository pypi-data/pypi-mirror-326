...
class SaleboxShippingOptions:
    ...
    def go(self, request, checkout, context):
                ...
                self.meta["items"].append(
                    {
                        ...
                        "width": b["variant"].get(self.SHIPPING_WIDTH, 0),
                        "height": b["variant"].get(self.SHIPPING_HEIGHT, 0),
                        "depth": b["variant"].get(self.SHIPPING_DEPTH, 0),
                        "weight": b["variant"].get(self.SHIPPING_WEIGHT, 0),
                    }
                )

                if b["variant"]["preorder_flag"]:
                    self.meta["total_items_preorder"] += 1
                    self.meta["total_weight_preorder"] += (
                        b["variant"][self.SHIPPING_WEIGHT] or 0
                    )
                    ...
                    self.meta["total_weight_onshelf"] += (
                        b["variant"][self.SHIPPING_WEIGHT] or 0
                    )
        ...
        if self.checkout["shipping_method"]["id"] is not None:
            option = next(
                o for o in opts if o["id"] == self.checkout["shipping_method"]["id"]
            )
    ...
    def init_option(self, id, code, label, remarks, service, meta=None):
        return {
            "available": True,
            "code": code,
            "id": id,
    ...
    def _example_option_1(self):
        method = self.init_option(
            1, "postoffice", "Post Office", "2-4 days", "Surface mail"
        )
    ...
    def _example_option_2(self):
        method = self.init_option(
            2, "postofficeexpress", "Post Office", "1 day", "NextDay"
        )
