...
from saleboxdjango.lib.common import fetchsinglevalue, dictfetchall, get_rating_display
...
class SaleboxProduct:
    ...
    def get_list(self, request=None):
        ...
        if data is None:
            ...
            self.query = (
                self.query.exclude(product__id__in=self.exclude_product_ids)
            ...
            data = {
                ...
                "qs": self._retrieve_results(
                    variant_ids, preserve_order=self.order_related is not None
                ),
            }
    ...
    def get_single(self, request, id, slug):
        self.query = (
            self.query.filter(id=id).filter(slug=slug).values_list("id", flat=True)
        )
        ...
        return self.retrieve_user_interaction(
            request, self._retrieve_results(variant_ids)
        )[0]
    ...
    def retrieve_user_interaction(self, request, variants):
        ...
        if self.fetch_user_ratings and request.user.is_authenticated:
            ratings = ProductVariantRating.objects.filter(
                variant__id__in=[pv.id for pv in variants]
        ...
        for pv in variants:
            if str(pv.id) in request.session["saleboxbasket"]["basket"]["lookup"]:
                pv.basket_qty = request.session["saleboxbasket"]["basket"]["lookup"][
                    str(pv.id)
                ]["qty"]
            else:
                pv.basket_qty = 0
            ...
            pv.in_wishlist = (
                str(pv.id) in request.session["saleboxbasket"]["wishlist"]["order"]
            )
    ...
    def set_prefetch_variant_attributes(self, numbers):
        if isinstance(numbers, int):
            numbers = [numbers]
        self.prefetch_variant_attributes = ["attribute_%s" % i for i in numbers]
    ...
    def set_category(self, category, include_child_categories=True):
        if include_child_categories:
            id_list = category.get_descendants(include_self=True).values_list(
                "id", flat=True
            )
        else:
    ...
    def set_order_related(self, root_category, fields, string=None, string_weight=10):
        self.order_related = {
            "category_ids": list(
                root_category.get_descendants(include_self=True).values_list(
                    "id", flat=True
                )
            ),
            "fields": fields,
            "string": {"string": string, "weight": string_weight},
        }
    ...
    def set_order_related_from_variant(self, variant, fields):
        ...
        for f in fields:
            ...
            if f["type"] == "product":
                f["values"] = list(
                    getattr(variant.product, "attribute_%s" % f["id"])
                    .all()
                    .values_list("id", flat=True)
                )
            if f["type"] == "variant":
                f["values"] = list(
                    getattr(variant, "attribute_%s" % f["id"])
                    .all()
                    .values_list("id", flat=True)
                )
        ...
        self.set_order_related(
            variant.product.category.get_root(), fields, variant.name
        )
    ...
    def set_product_attribute_include_keyvalue(
        self, attribute_number, field_name, field_value, field_modifier=None
    ):
    ...
    def _retrieve_results(self, variant_ids, preserve_order=False):
        qs = []
        if len(variant_ids) > 0:
            qs = ProductVariant.objects.filter(id__in=variant_ids).select_related(
            ...
            if self.flat_discount > 0:
                ...
                qs = qs.annotate(modified_price=F("price") * ratio)
            ...
            if self.flat_member_discount > 0:
                ...
                qs = qs.annotate(
                    modified_price=Case(
                        When(member_discount_applicable=True, then=F("price") * ratio),
                        default=F("price"),
                    )
                )
            ...
            if preserve_order:
                ...
                if self.flat_discount > 0 or self.flat_member_discount > 0:
                    self.order = [
                        o.replace("sale_price", "modified_price") for o in self.order
                    ]
    ...
    def _retrieve_variant_ids(self, do_exclude=False):
        ...
        if do_exclude:
            ...
            with connection.cursor() as cursor:
                cursor.execute(sql)
                self.set_exclude_productvariant_ids(
    ...
    def _sort_by_related_items(self, variant_ids):
        ...
        for f in self.order_related["fields"]:
            if f["type"] == "category":
                subquery.append(
                    "(CASE WHEN p.category_id = %s THEN %s ELSE 0 END)"
                    % (f["values"][0], f.get("weight", 1))
                )
            if f["type"] == "product":
                subquery.append(
                    "((SELECT COUNT(*) FROM saleboxdjango_product_attribute_%s WHERE product_id = p.id AND attributeitem_id IN (%s)) * %s)"
                    % (
                        f["id"],
            ...
            if f["type"] == "variant":
                subquery.append(
                    "((SELECT COUNT(*) FROM saleboxdjango_productvariant_attribute_%s WHERE productvariant_id = p.id AND attributeitem_id IN (%s)) * %s)"
                    % (
                        f["id"],
                        ",".join(str(i) for i in f["values"]),
                        f.get("weight", 1),
        ...
        if len(self.order_related["category_ids"]) > 0:
            sql = sql.replace(
                ...
                % ",".join([str(i) for i in self.order_related["category_ids"]]),
            )
        else:
