...
class ExcelProcessor:
    def __init__(self, db_config: dict):
        ...
        # 定义关键词到表名的映射
        self.table_mapping = {
            ('销售', 'sell', 'sale', '销量'): 'sales_detail',
            ('门店', 'store', 'shop', '分店', '分公司'): 'store_info',
            ('目标', 'target', '指标', '任务'): 'store_target',
            ('产品', 'product', 'item', '商品', '货品'): 'product_mapping',
            ('库存', 'inventory', 'stock', '仓储'): 'physical_inventory',
            ('战役', 'campaign', '活动', '促销'): 'campaign_target',
            ('服务', 'service', '维修', '保修'): 'service_mapping',
            ('验机', 'device', '设备', '机器'): 'device_acceptance',
            ('订单', 'order', '交易'): 'order_detail',
            ('客户', 'customer', '会员', 'vip'): 'customer_info',
            ('员工', 'staff', '人员', '雇员'): 'staff_info',
            ('供应商', 'supplier', 'vendor'): 'supplier_info'
        }
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 先处理数值类型的列
        numeric_cols = df.select_dtypes(include=['int64', 'Int64', 'float64', 'Float64']).columns
        ...
        # 处理其他列
        other_cols = df.select_dtypes(exclude=['int64', 'Int64', 'float64', 'Float64', 'datetime64']).columns
