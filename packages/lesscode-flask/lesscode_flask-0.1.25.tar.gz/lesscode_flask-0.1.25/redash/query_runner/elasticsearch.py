...
class BaseElasticSearch(BaseQueryRunner):
    ...
    def create_options(self, configuration):
        options = {
            "server": f'http://{configuration.get("host")}:{configuration.get("port")}/{configuration.get("dbname")}',
            "basic_auth_user": configuration.get("username"),
            "basic_auth_password": configuration.get("password")
        }
