...
def main(
    ...
    if get_opened_file_list(change_obj.change):
        ...
        if use_server_submit and commit_on_success:
            ...
            revert_file_in_cl("//...", change_obj.change, "-k")
    else:
    ...
    build_id = get_shelve_preflight_build_id(
        shelve_changelist, type=type, stream=get_client_stream(change_obj.client), submit_on_server=(use_server_submit and commit_on_success), content_only_build=use_content_build
    )
