...
class Tunnel:
    ...
    def __init__(self, port: int, subdomain: Optional[str] = None, dev_mode: bool = False) -> None:
        ...
        # Start monitoring and WebSocket connection
        self._start_monitor()
        self._start_tunnel()

        logger.info(f"Tunnel started at {self.public_url}")
    ...
    def _start_monitor(self) -> None:
        """Monitor local server availability in a background thread."""
        def monitor():
    ...
    def _validate_subdomain(self, subdomain: str) -> str:
        ...
        if len(subdomain) < self.MIN_SUBDOMAIN_LENGTH or len(subdomain) > self.MAX_SUBDOMAIN_LENGTH:
            raise TunnelError(f"Subdomain must be between {self.MIN_SUBDOMAIN_LENGTH} and {self.MAX_SUBDOMAIN_LENGTH} characters")
    ...
    def _start_tunnel(self) -> None:
        """Start tunnel WebSocket connection in a background thread."""
        def run():
            while not self._shutdown.is_set():
                try:
