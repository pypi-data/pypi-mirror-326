...
from malevich_app.export.secondary.helpers import get_collection_pandas, get_df_object, get_collection_by_id, get_collection_json, save_collection, save_doc,\
    DocsRaw, VariantList
...
class JuliusCollectionHolder:   # TODO gc
    ...
    async def __update_collection(self, id: int, collection_id: str, scheme: Optional[str], force_reload: bool = False):
        ...
        if self.__is_local:
            ...
            collection_and_scheme = CollectionAndScheme(operationId=self.__operation_id, collectionId=collection_id, hostedAppId=APP_ID, secret=self.__secret, fixScheme=fix_scheme)
            res = await post_request_json(MONGO_LOAD(self.__dag_host_port), collection_and_scheme.json(), buffer=self.__logs_buffer, fail_info=f"{LOAD_COLLECTION_FAIL_MSG}: {collection_id}")
