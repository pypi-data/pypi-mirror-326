...
from malevich_app.export.secondary.endpoints import QUERY, CREDENTIALS, OUTSIDE_COLLECTION, RUN_SCHEME
...
class JuliusApp:
    ...
    @property
    def fun_arguments(self) -> Tuple[str, ...]:
        if self.__processor_fun is not None:
            return get_argnames(self.__processor_fun)
        return get_argnames(self.__condition_fun)
    ...
    def get_scale_part(self, df: pd.DataFrame) -> pd.DataFrame:
        return self._context.get_scale_part(df)
        ...
        try:
            if isinstance(coll, str):
                ...
                if "credentials_id" in coll:
                    ...
                    coll = await post_request_json(CREDENTIALS, credentials.json(), buffer=self.logs_buffer)
                ...
                coll_id = await post_request_json(OUTSIDE_COLLECTION, collection_outside.json(), text=True, buffer=self.logs_buffer)
                coll_mode = coll.get("drop_mode", "check")
    ...
    async def __update_schemes_inside(self, scheme_name: Optional[str]) -> Optional[str]:
        if scheme_name is None or scheme_name == "*":
            ...
            if not self.__is_local:
                await post_request_json(RUN_SCHEME, temp_run_scheme.json(), text=True, buffer=self.logs_buffer)
    ...
    def register_input(self, input_fun: callable, id: Optional[str], collection_from: Optional[str], collections_from: Optional[List[str]], extra_collection_from: Optional[str], extra_collections_from: Optional[List[str]], by_args: bool, query: Optional[str], finish_msg: Optional[str], mode: InputWrapper):
        if id is None:
        ...
        if self.__mode == "info":
            ...
            if id not in self.__info.inputs:
                input_function_info = InputFunctionInfo(id=id, name=input_fun.__name__, arguments=get_fun_info_verbose(input_fun), doc=input_fun.__doc__, collectionsNames=collections_names, extraCollectionsNames=extra_collections_names, query=query, finishMsg=finish_msg, mode=mode.value)
        ...
        if query is not None:
            ...
            collection_from = asyncio.run(post_request_json(QUERY, query_collection.json(), text=True, buffer=self.logs_buffer))
        self.__input_collection_from = collection_from
        self.__input_collections_from = collections_from
        self.__input_extra_collection_from = extra_collection_from
        self.__input_extra_collections_from = extra_collections_from
    ...
    def register_processor(self, processor_fun: callable, id: Optional[str], finish_msg: Optional[str], drop_internal: bool, get_scale_part_all: bool = False):
        if id is None:
        ...
        if self.__mode == "info":
            if id not in self.__info.processors:
                ...
                processor_function_info = ProcessorFunctionInfo(id=id, name=processor_fun.__name__, arguments=get_fun_info_verbose(processor_fun), doc=processor_fun.__doc__, finishMsg=finish_msg, contextClass=get_context_info(schemes_info[0]))
    ...
    def register_output(self, output_fun: callable, id: Optional[str], collection_out_name: Optional[str], collection_out_names: Optional[List[str]], finish_msg: Optional[str], drop_internal: bool):
        if id is None:
        ...
        if self.__mode == "info":
            if id not in self.__info.outputs:
                output_function_info = OutputFunctionInfo(id=id, name=output_fun.__name__, arguments=get_fun_info_verbose(output_fun), doc=output_fun.__doc__, collectionOutNames=collection_out_names, finishMsg=finish_msg)
    ...
    def register_condition(self, condition_fun: callable, id: Optional[str], finish_msg: Optional[str], drop_internal: bool):
        if id is None:
            id = condition_fun.__name__
        if self.__mode == "info":
            if id not in self.__info.conditions:
                condition_function_info = ConditionFunctionInfo(id=id, name=condition_fun.__name__, arguments=get_fun_info_verbose(condition_fun), doc=condition_fun.__doc__, finishMsg=finish_msg)
    ...
    def register_init(self, init_fun: callable, id: Optional[str], enable: bool, tl: Optional[int], prepare: bool):
        ...
        if self.__mode == "info":
            self.__info.inits[id] = InitInfo(id=id, enable=enable, tl=tl, prepare=prepare, argname=get_context_argname(init_fun), doc=init_fun.__doc__)
        if not enable:
            return
        ...
        if self.__app_version >= 2:
            if prepare:
                self.__init_funs_first.append(InitFunState(self, init_fun, id, tl))
            else:
                self.__init_funs.append(InitFunState(self, init_fun, id, tl))
        else:
            self.__init_funs.append(InitFunState(self, init_fun, id, tl))
    ...
    async def __input(self, *args, **kwargs):
        try:
            res = await run_func(self.__input_fun, *args, **kwargs)
    ...
    async def __processor(self, *args, **kwargs):
        try:
            res = await run_func(self.__processor_fun, *args, **kwargs)
    ...
    async def __output(self, *args, **kwargs):
        try:
            res = await run_func(self.__output_fun, *args, **kwargs)
    ...
    async def __condition(self, *args, **kwargs):
        try:
            res = await run_func(self.__condition_fun, *args, **kwargs)
    ...
    def _cfg_dict_if_need(self, fun_id: Optional[str] = None) -> Dict[str, Any]: # maybe cfg is wrong because it is already used
        ...
        if isinstance(app_cfg, BaseModel):
            app_cfg = app_cfg.dict()
        return app_cfg

    async def run(self, *args, **kwargs) -> Tuple[bool, Any]:
        ...
        out = sys.stdout
        sys.stdout = self.logs_buffer
        res = await run_fun(*args, **kwargs)
        sys.stdout = out
