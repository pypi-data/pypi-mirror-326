...
class KafkaHelper:  # TODO add custom timeout
    ...
    async def run(self):
        ...
        try:
            ...
            await send_post_dag(kafka_info.json(), KAFKA_INFO(self.__j_app.dag_host_port))
    ...
    async def produce(self, dfs_schemes: List[Tuple[Union[pd.DataFrame, Type[BaseModel]], Optional[str]]]) -> List[Collection]:
        ...
        if self.__send_topics is None:  # only in the end
            msg = FinishKafkaMsg(operationId=self.__j_app.operation_id, runId=self.__j_app.run_id, data=results).json()
            ...
            msg = KafkaAppMsg(operationId=self.__j_app.operation_id, runId=self.__j_app.run_id, data=results, schemes=schemes,
                              index=self.__index, taskId=self.__j_app.task_id, appId=self.__j_app.app_id).json()
