...
from malevich_app.export.abstract.abstract import Collection as Coll, Objects, FixScheme
...
def send_background_task(fun: callable, *args, **kwargs):
    lambda_fun = (lambda: asyncio.run(fun(*args, **kwargs))) if is_async(fun) else (lambda: fun(*args, **kwargs))
    task = asyncio.create_task(run_in_threadpool(lambda_fun))
...
def save_collection(df: pd.DataFrame, operation_id: str, fix_scheme: Optional[FixScheme] = None) -> Collection:
    ...
    df["__name__"] = df.apply(lambda _: "", axis=1)
    df["__id__"] = df.apply(lambda _: str(uuid4()), axis=1)
    collection_id = save_collection_pandas(df, operation_id, None if fix_scheme is None else fix_scheme.dict()) # ...
...
def save_collection_external(collection: Coll):
    ...
    if is_doc:
        doc = json.loads(collection.data)
        save_collection_json(doc, collection.operationId, None if collection.scheme is None else collection.scheme.dict(), coll_id=collection.id)
    else:
        df = pd.read_json(collection.data)  # FIXME not only pandas
        fix_df_id_name(df)
        save_collection_pandas(df, collection.operationId, None if collection.scheme is None else collection.scheme.dict(), coll_id=collection.id)
...
def save_doc(doc: Union[Doc, Docs], operation_id: str, fix_scheme: Optional[FixScheme]) -> Collection:
    collection_id = save_collection_json(doc.json(), operation_id, None if fix_scheme is None else fix_scheme.dict())
...
async def reverse_object_request(objects: Objects, dag_host_port: str, logs_buffer: io.StringIO) -> bool:
    ...
    if objects.runId is not None:
        url += f"/{objects.runId}"
    try:
        await send_post_dag(data, url, headers={'Content-type': 'application/octet-stream'})    # TODO secret
    except:
...
async def save_collections(julius_app, collections: List[Collection]):   # not for base apps
    ...
    if len(main_collections) > 0:
        ...
        if data is not None and len(data) > 0:
            tasks.append(send_post_dag(data, f"{COLLECTIONS(julius_app.dag_host_port)}/{C.APP_ID}/{julius_app.operation_id}/false", headers={'Content-type': 'application/octet-stream'}))
    if len(new_obj_collections) > 0:
        ...
        if data is not None and len(data) > 0:
            tasks.append(send_post_dag(data, f"{COLLECTIONS(julius_app.dag_host_port)}/{C.APP_ID}/{julius_app.operation_id}/true", headers={'Content-type': 'application/octet-stream'}))
    if len(tasks) > 0:
