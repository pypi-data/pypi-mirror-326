...
class SNN(bp.DynamicalSystem):
  def __init__(self, num_in, num_rec, num_out):
    ...
    # neuron groups
    self.i = bp.dyn.LifRef(num_in, tau=10., V_reset=0., V_rest=0., V_th=1., R=1.,spk_reset = 'hard',tau_ref=0.,method='euler')
    self.r = bp.dyn.LifRef(num_rec, tau=10., V_reset=0., V_rest=0., V_th=1., R=1.,spk_reset = 'hard',tau_ref=0.,method='euler')
    self.o = bp.dyn.LifRef(num_out, tau=10., V_reset=0., V_rest=0., V_th=1., R=1.,spk_reset = 'hard',tau_ref=0.,method='euler')
...
if __name__ == "__main__":
  ...
  outs = runner.run(inputs=inpS) # must set reset_state=True if jit is enabled 
  outs = outs[-1,:,:]
