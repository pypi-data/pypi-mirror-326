...
class SNN(bp.DynamicalSystem):
  def __init__(self, num_in, num_rec, num_out):
    ...
    # neuron groups
    self.r = bp.dyn.Lif(num_rec, tau=10., V_reset=0., V_rest=0., V_th=1.,method="euler",spk_reset = 'hard')
    self.o = bp.dyn.Integrator(num_out, tau=10.)

    # synapse: i->r
    self.i2r = bp.Sequential(
        comm=bp.dnn.Linear(num_in, num_rec, W_initializer=bp.init.KaimingNormal(scale=20.), b_initializer=None),
        syn=bp.dyn.Expon(num_rec, tau=10.,method="euler"),
    )
