...
class DataProcessor(BaseProcessor):
        ...
        try:
            ...
            with mlflow.start_run(nested=True):
                with tempfile.TemporaryDirectory() as tmp_dir:
                    scaler_path = Path(tmp_dir) / "scaler"
                    mlflow.sklearn.save_model(self.scaler, scaler_path)
    ...
    def calculate_training_stats(self, X_train: pd.DataFrame) -> Dict[str, Any]:
        ...
        # mlflow.log_artifact(str(stats_path))  # TODO: fix parent / child run clash
        return stats
...
class ModelTrainer(BaseTrainer):
    ...
    def train_models(self, random_state: int = 42) -> None:
        try:
            ...
            with mlflow.start_run(run_name="training_session", nested=True) as parent_run:
                mlflow.log_params(
                    {
