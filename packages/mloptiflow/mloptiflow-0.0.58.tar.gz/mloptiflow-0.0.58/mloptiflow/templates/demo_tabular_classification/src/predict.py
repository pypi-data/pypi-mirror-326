...
class ModelPredictor:
    def __init__(
        self,
        experiment_name: str = "demo_tabular_classification",
    ...
    def _load_artifacts(self):
        try:
            model_info = self.registry.get_latest_model(self.experiment_name, self.model_name)
            self.model = mlflow.sklearn.load_model(str(model_info["model_path"]))
            self.scaler = joblib.load(str(model_info["scaler_path"]))
