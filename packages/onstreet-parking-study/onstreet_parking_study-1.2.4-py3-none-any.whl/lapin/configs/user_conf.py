...
##################
class LapinConfig:
    """Configurations option for LAPIN analysis.
...
class UserConfig(LapinConfig):
    ...
    def __init__(self, config_name, encoding="latin-1"):
        ...
        if json_covertible(config_name, encoding):
            for section, value in json_covertible(config_name, encoding).items():
                if isinstance(value, dict):
                    self.config[section] = value
                else:
                    raise ValueError(
                        f"Dict value was expected for Section {section} but received {type(value)}",
                    )
        else:
