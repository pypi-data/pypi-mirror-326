...
class SegmentDivision:
    ...
    def get_active_reg(
        self, time: np.datetime64 = None, ignore_reg: list = None
    ) -> Regulation:
    ...
    def from_json(cls, regulation: dict):
        ...
        if (  # there is a <<simple>> regulation
            definition["interdiction_complete"] or not definition["restrictions"]
        ) and (  # this regulation has been entered by a human
            definition.get("type_interdiction", None)
            or definition["restrictions"]
            or definition["completed"]
            or definition["user_modified"]
        ):
        ...
        extended_attributes = definition.get(
            "extended_attributes", {"number_of_places": ""}
        )
        # this is a blank restriction
        if (
            not definition["restrictions"]
            and extended_attributes["number_of_places"] == ""
        ):
            return
