...
from lapin.tools.utils import parse_days, DAYS, DAYS_MAP
...
class Scheduler:
    ...
    @classmethod
    def from_datas(cls, conf: UserConfig, analysis_data: SchedulerDataStore):
        """_summary_
        ...
        _ = study.merge_lpr_readings(
            analysis_data.lpr_data[
                ["segment", "side_of_street", "uuid", "datetime", "plaque"]
            ]
        )
        study.update_with_config(network_config, json_keys_2_int(conf.vehicule_conf))
    ...
    def _vis_delim_iterable(self) -> dict:
        ...
        if isinstance(self.datastore.vis_delim, pd.core.series.Series):
            if self.config.gis_vis_bounds_names in self.datastore.vis_delim.index:
                vis_delim = {
            ...
            if self.config.gis_vis_bounds_names in self.datastore.vis_delim.columns:
                if self.datastore.vis_delim.shape[0] > 1:
                    ...
                    vis_delim = {
                        self.datastore.vis_delim[self.config.gis_vis_bounds_names]
                        .iloc[0]: self.datastore.vis_delim["geometry"]
                        .iloc[0]
                    }
