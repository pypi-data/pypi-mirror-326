...
To use the GithubSearchTool, first ensure the cropioai_tools package is installed in your Python environment:

```shell
pip install 'cropioai[tools]'
```

This command installs the necessary package to run the GithubSearchTool along with any other tools included in the cropioai_tools package.
...
```python Code
from cropioai_tools import GithubSearchTool
