...
from cropioai.knowledge.source.text_file_knowledge_source import TextFileKnowledgeSource


@pytest.fixture(autouse=True)
def mock_vector_db():
    """Mock vector database operations."""
    with patch("cropioai.knowledge.storage.knowledge_storage.KnowledgeStorage") as mock:
...
def test_pdf_knowledge_source(mock_vector_db):
    ...
    pdf_path = current_dir / "cropioai_quickstart.pdf"
    ...
    mock_vector_db.query.return_value = [
        {"context": "cropioai create cropio latest-ai-development", "score": 0.9}
    ]

    # Perform a query
    query = "How do you create a cropio?"
    results = mock_vector_db.query(query)

    # Assert that the correct information is retrieved
    assert any(
        "cropioai create cropio latest-ai-development" in result["context"].lower()
        for result in results
    )
    mock_vector_db.query.assert_called_once()
...
def test_docling_source(mock_vector_db):
    docling_source = CropioDoclingSource(
        file_paths=[
...
def test_multiple_docling_sources():
    ...
    docling_source = CropioDoclingSource(file_paths=urls)
