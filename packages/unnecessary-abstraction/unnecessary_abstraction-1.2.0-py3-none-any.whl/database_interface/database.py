...
class Database(Protocol):
    ...
    def select(self, table_name:str, cols:list[str]=["*"], where_clause:Where=None) -> list[tuple]:
        ...
    ...
    def join(self, left_table:str, right_table:str, left_on:str, right_on:str, return_cols:list[str]=["*"], type:JOIN_TYPE="INNER", where:Where=None):
        ...
    def delete(self, table_name:str, where_clause:Where) -> None:
    ...
    def table_to_records(self, table_name:str, cols:list[str]=["*"], where_clause:Where=None) -> list[dict]:
        schema = self.get_schema(table_name, cols)
        if not schema:
            raise DatabaseError(Error.TABLE_DOESNT_EXIST, table_name=self.table(table_name))
        
        table_data = self.select(table_name, cols, where_clause)
