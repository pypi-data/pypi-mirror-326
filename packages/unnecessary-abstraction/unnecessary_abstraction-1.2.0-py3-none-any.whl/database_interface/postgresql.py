...
from db_interface.sql_objects import Select, Where, Update, Insert, Join, JOIN_TYPE
...
class PostgreSQL(Database):
    ...
    def upsert(self, table_name:str, cols:list[str], rows:list[tuple], id_col_name:str) -> None:
        schema = self.get_schema(table_name, cols)
        rows = self.convert_types(schema, rows, "insert")
        query, params = Insert(self.table(table_name), cols).values(rows).update_on_conflict(id_col_name, "PostgreSQL").build(self.__binding_char)
        with self.cursor() as cur:
            cur.executemany(query, params)
