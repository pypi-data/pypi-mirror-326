...
class ServicesBus:
    ...
    @staticmethod
    async def send_message(conn_str:str, queue_name:str, message: dict | str) -> None:
        """Method to send a message to the Service Bus."""
        try:
            data = json.dumps(message) if isinstance(message, dict) else message
            async with ServiceBusClient.from_connection_string(conn_str) as servicebus_client:
