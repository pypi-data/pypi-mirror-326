...
class DbCursor:
    ...
    def sqlFormat(self, sql: str, args: Sequence[Any]) -> str:
        sql = self._sqlFormatRe.sub(' ', sql).strip()
        if '%x' in sql:
            sql = sql.replace('%x', self.makePlacement(args))
        return sql

    def makePlacement(self, value: int | Sequence[Any] | set[Any]) -> str:
...
class SqliteCursor(DbCursor):
    ...
    def sqlFormat(self, sql: str, args: Sequence[Any]) -> str:
        sql = sql.replace('%s', '?')
        return super().sqlFormat(sql, args)
    ...
    async def saveList(self, modelList: Sequence[BModel], *, tableName: str = ''):
        ...
        for model in modelList:
            values.append(tuple(model.model_dump().values()))
        tableName = tableName or modelList[0].__class__.tableName()
        sql = f'''INSERT OR REPLACE INTO `{tableName}` ( {','.join(columns)} ) VALUES ( %x )'''
        sql = self.sqlFormat(sql, values[0])
        return await self.cursor.executemany(sql, values)
