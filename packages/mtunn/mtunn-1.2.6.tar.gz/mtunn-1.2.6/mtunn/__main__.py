...
try:
    ...
    import http.client, urllib.parse, json, ssl, base64, re
    import os, sys, time, platform, threading, signal, argparse, curses, shutil
    import socket, json, ipaddress, zlib, itertools, ctypes, gzip, struct, lockis
    ...
    if "curses" in str(e):
        ...
        try:
            ...
            print("“lockis” has been successfully installed")
            print("please run «mtunn» again")
        except subprocess.CalledProcessError as e:
...
def ipv():
    global support_ipv4
    global support_ipv6
    if support_ipv4 == False and support_ipv6 == False:
        support_ipv4 = False
        support_ipv6 = False
        ipv4 = ["ipv4.google.com", "ipv4.lookup.test-ipv6.com"]
        ipv6 = ["ipv6.google.com", "ipv6.lookup.test-ipv6.com"]
        for url in ipv4:
            try:
                ...
                conn = http.client.HTTPSConnection(url, timeout=5, context=context)
                conn.request("GET", "/", headers={"User-Agent": "Mozilla/5.0 (X11; Debian; Linux x86_64; rv:60.0) Gecko/20100101 Firefox/60.0", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8", "Accept-Language": "en-US,en;q=0.5", "Accept-Encoding": "identity", "Connection": "close"})
...
def read_configs():
    global tunnel_conf, tunnel_cert
    if platform.system().lower() == "windows":
...
def create_connection(host, port, use_https, timeout):
    ...
    global support_ipv6
    if support_ipv6:
        ip = send_dns_query(host, 28)[0]
    else:
        ip = send_dns_query(host, 1)[0]
...
def rpost(url, json_data=None, timeout=10):
    parsed_url = urllib.parse.urlparse(url)
    use_https = parsed_url.scheme == "https"
    port = parsed_url.port or (443 if use_https else 80)
    conn = create_connection(parsed_url.hostname, port, use_https, timeout)
...
def tunnels():
    ...
    try:
        response = rget("https://raw.githubusercontent.com/mishakorzik/mtunn/refs/heads/main/tunnels.json", timeout=10)["response"]
        hostname = []
...
def resolve_tunnel(tunnel, method="A"): # resolve tunnel domain using github
    try:
        get = rget("https://raw.githubusercontent.com/mishakorzik/mtunn/refs/heads/main/tunnels.json", timeout=10)["response"]["tunnels"]
        for check in get:
...
def resolve_domain(domain, method="A"): # resolve domain using dns
    if method == "A":
        return send_dns_query(domain, 1)[0]
    elif method == "AAAA":
        return send_dns_query(domain, 28)[0]
    return None

def certs(main_server): # check server status and ssl certificate
    global tunnel_cert
    try:
        post = rpost(f"https://{main_server}:5569/status", json_data={"id": 0}, timeout=10)["response"]
        if post["server"] == "ok" and post["security"] in ("TLSv1.3", "TLSv1.2"):
            return True
        else:
            return False
    except (ssl.SSLCertVerificationError, ssl.SSLError, KeyError, TypeError):
        try:
            get = rget(f"https://raw.githubusercontent.com/mishakorzik/mtunn/refs/heads/pages/certs/{main_server}.txt", timeout=10)["response"]
            with open(tunnel_cert, "w") as file:
                file.write(get)
            post = rpost(f"https://{main_server}:5569/status", json_data={"id": 0}, timeout=10)["response"]
            if post["server"] == "ok" and post["security"] in ("TLSv1.3", "TLSv1.2"):
...
def set_net(buffer): # set buffer size
    if platform.system().lower() == "linux":
        os.system("sudo sysctl -w net.core.rmem_default=131072")
        os.system("sudo sysctl -w net.core.wmem_default=65536")
        os.system("sudo sysctl -w net.core.rmem_max={str(buffer)}")
        os.system("sudo sysctl -w net.core.wmem_max={str(buffer // 2)}")
        return True
    elif platform.system().lower() in ("freebsd", "netbsd", "openbsd"):
...
def get_net(): # get buffer sizes (recv, send)
    try:
        recv, send = 0, 0
...
def mtunn_path(): # config file locations
    ...
    elif platform.system().lower() in ("openbsd", "freebsd", "netbsd", "darwin"):
...
def account(stdscr, path):
    with open(path, "r") as file:
        ...
        email = data[1]
        main_server = data[2]
    post = rpost(f"https://{main_server}:5569/auth/register_date", json_data={"email": email}, timeout=10)["response"]
    if post["status"] == "success" and "rd:" in post["message"]:
        date = post["message"][3:]
    else:
        date = "unknown"
    post = rpost(f"https://{main_server}:5569/auth/total_quota", json_data={"token": token}, timeout=10)["response"]
    if post["status"] == "success":
        payouts = str(post["payouts"])
    else:
        payouts = "?"
    post = rpost(f"https://{main_server}:5569/auth/get_quota", json_data={"token": token}, timeout=10)["response"]
    if post["status"] == "success":
    ...
    if key.lower() == "yes, delete my account.":
        try:
            with open(path, "r") as file:
                ...
                email = data[1]
                main_server = data[2]
            post = rpost(f"https://{main_server}:5569/auth/delete_account", json_data={"token": token, "email": email}, timeout=10)["response"]
            if post["status"] == "success":
...
def change_email(stdscr, path):
    ...
    curses.curs_set(0)
    try:
        post = rpost(f"https://{main_server}:5569/auth/change_email", json_data={"new_email": new_email, "old_email": old_email, "token": token}, timeout=10)["response"]
        if post["status"] == "success":
            ...
            post = rpost(f"https://{main_server}:5569/auth/verify_account", json_data={"email": new_email, "code": code}, timeout=10)["response"]
...
def register(stdscr, path, main_server):
    ...
    while True:
        ...
        if email in [curses.KEY_UP, curses.KEY_DOWN, curses.KEY_LEFT, curses.KEY_RIGHT] or email == "":
            ...
            break
    try:
        post = rpost(f"https://{main_server}:5569/auth/check", json_data={"email": email}, timeout=10)["response"]
        if post["status"] == "success":
            if post["message"] == "x01":
                post = rpost(f"https://{main_server}:5569/auth/register", json_data={"email": email}, timeout=10)["response"]
            elif post["message"] == "x03":
                post = rpost(f"https://{main_server}:5569/auth/login", json_data={"email": email}, timeout=10)["response"]
            else:
                line = 6
            ...
            if post["status"] == "success":
                ...
                stdscr.refresh()
                line = 7
                post = rpost(f"https://{main_server}:5569/auth/verify_account", json_data={"email": email, "code": code}, timeout=10)["response"]
                if post["status"] == "success" and "token:" in post["message"]:
                    with open(path, "w") as file:
                        file.write(post["message"].replace("token:", "")+"\n")
                        file.write(email+"\n")
...
def cquota(stdscr, path):
    ...
    with open(path, "r") as file:
        ...
        main_server = data[2]
    try:
        post = rpost(f"https://{main_server}:5569/auth/count_quota", json_data={"conn": conn, "tunn": tunn, "netw": netw, "prun": tta}, timeout=10)["response"]
    except:
    ...
    if post.get("status") == "success":
        ...
        if key.lower() == "yes, accept.":
            try:
                post = rpost(f"https://{main_server}:5569/auth/change_quota", json_data={"token": token, "conn": conn, "tunn": tunn, "netw": netw, "prun": prun}, timeout=10)["response"]
                if post.get("status") == "success":
...
def check_domain(custom_domain):
    ...
    global tunnel_domain
    while show_message:
        if support_ipv6 == True and isinstance(ipaddress.ip_address(tunnel_address), ipaddress.IPv6Address) == True:
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    if sock.recv(4) == b'pong':
                        latency = str(round((time.time() - start_time) * 1000, 1)) + "ms"
                    sock.close()
            except:
                pass
...
class SockIO(object):
    ...
    def send(self, data):
        pack = memoryview(self.pi.make(data))
        with self.send_lock:
            try:
...
class Runable(Base):
    ...
    def _dog_run(self):
        global status, latency, tunnel_conn, max_network, packet_loss
...
class Client(SockRunable):
    def __init__(self, **cxt):
        ...
        self.buffer = cxt.get('buffer', 131072)
        self.token = cxt.get('token')
        self.proto = cxt.get('proto', 'http')
        ...
        if platform.system().lower() in ("linux", "openbsd", "freebsd", "netbsd"):
    ...
    def _add_con(self, ix):
        try:
            if support_ipv6 == True and isinstance(ipaddress.ip_address(self.target_host), ipaddress.IPv6Address) == True:
                sock = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
                sock.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_UNICAST_HOPS, 128)
            else:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    def _run_ucheck(self):
        ...
        while self._running:
            try:
                post = rpost(f"https://{self.primary}:5569/version", json_data={"type": "latest-stable"}, timeout=10)["response"]
    ...
    def _run_p(self, package):
        global tunnel_conn, tunnel_total, quota_new, used_network, all_conn, tunnel_traffic
        try:
            ix, data = parse_package(package)
            if ix != 0:
                tunnel_traffic += len(package)
            del package
            if ix == 0:
                if data.startswith(b";"):
                    self.feed_dog()
                    ac, dec, rq, tc = data.decode('utf-8')[1:].split("/")
                    tunnel_conn = int(tc)
                    tunnel_total = str(ac)
                    quota_new = int(rq)
                    ...
                    try:
                        data = self.secret.decrypt(data, ttl=10)
    ...
    def _run(self):
        ...
        global used_network, max_network, tunnel_traffic, tunnel_conn, quota_new
        ...
        try:
            if support_ipv6 == True and isinstance(ipaddress.ip_address(tunnel_address), ipaddress.IPv6Address) == True:
                ...
                sock.connect((tunnel_address, 5567))
                self._sock = sock
            self.secret = lockis.gkey()
            self.feed_dog()
            send_package(sock, 0, encrypt_message(self.secret.decode('utf-8')))
            self.secret = lockis.lockis(self.secret)
            data = json.dumps(
            {
              "tunnel": {
                "buffer": self.buffer,
            ...
            send_package(sock, 0, self.secret.encrypt(data.encode('utf-8')))
            self.feed_dog()
            ret = json.loads(self.secret.decrypt(bytes(parse_package(read_package(sock))[1]), ttl=10).decode('utf-8'))
            if ret["s"] == 1:
                status = "online"
                server_version = str(ret["v"])
                server_build = str(ret["b"])
                tunnel_max_c = str(ret["mt"])
                used_network = int(ret["un"])
                max_network = int(ret["mn"])
                connections = int(ret["mc"])
                self._athread = ThreadPoolExecutor(max_workers=50+(connections*3))
            ...
            self.stop()
        self.feed_dog()
        start_thread(target=self._run_proc)
        start_thread(target=self._run_ucheck)
        while self._running:
            recv = read_package(self._sock)
            if recv:
                self._athread.submit(self._run_p, recv)

    def stop(self):
        if self.secret != None:
            try: send_package(self._sock, 0, self.secret.encrypt(b'close_tunnel'))
            except: pass
        self.stop_dog()
    ...
    def remote(self):
        ...
        for port in range(7010, 7091):
            try: s.bind(("127.0.0.1", int(port)))
            except: pass
        s.listen(3)
        while True:
            try:
                sock, addr = s.accept()
                threading.Thread(target=self.remote_sock, kwargs={"sock": sock}, daemon=True).start()
            except:
                pass

    def remote_sock(self, sock):
        global stop_tun
        global all_conn
        global ping_method
        global tunnel_address
        ...
        while True:
            ...
            if data["version"] == "mtunn_cv1.3":
                ...
                a2 = ""
                a3 = ""
                try: a1 = data["command"]["args1"]
                except: pass
                try: a2 = data["command"]["args2"]
                except: pass
                try: a3 = data["command"]["args3"]
                except: pass
                if r == "help":
...
\033[01;33mExamples:\033[0m
 ...
 conn a             : show all connections""")
                    ...
                    sock.send(json.dumps({"status": "success", "address": str(tunnel_address), "method": str(ping_method), "time": str(lat)}).encode())
                elif r == 'forward':
                    try:
                        sock.send(json.dumps({"status": "success", "remote": str(explore_domain)+":"+str(explore_port), "local": str(tunnel_two)}).encode())
                    except:
                        sock.send(json.dumps({"status": "error"}).encode())
                elif r == 'status':
                    if "online" in status:
                        sock.send(json.dumps({"status": "success", "tunnel": "online"}).encode())
                    elif "offline" in status:
                        sock.send(json.dumps({"status": "success", "tunnel": "offline"}).encode())
                    else:
                        sock.send(json.dumps({"status": "success", "tunnel": str(status)}).encode())
                elif r == 'list':
                    try:
                        sock.send(json.dumps({"status": "success", "list": nat_banned_ips}).encode())
                    except:
                        sock.send(json.dumps({"status": "error", "list": []}).encode())
                elif r == 'quota':
                    if str(quota_new) == "-" or str(max_network) == "-":
                        sock.send(json.dumps({"status": "success", "quota": "unknown"}).encode())
                    else:
                        sock.send(json.dumps({"status": "success", "quota": str(quota_new)}).encode())
                elif r == 'network':
                    _u = int(used_network)
                    _m = int(max_network)
                    if _u > 1024: # KBytes
                    ...
                    sock.send(json.dumps({"status": "success", "used": str(_u), "max": str(_m)}).encode())
                elif r.startswith('conn'):
                    if a1 == "all" or a1 == "a":
                        try:
                            wait = 0
                            current_time = time.time()
                            send_package(self._sock, 0, self.secret.encrypt(b'firewall;conn=all'))
                            time.sleep(1)
                            message_sent = False
                            while 7 > wait:
                                if all_conn == "nothing":
                                    if not message_sent:
                                        sock.send(json.dumps({"status": "error", "message": "no connections"}).encode())
                                        message_sent = True
                                    break
                                else:
                                    package = []
                                    ...
                                    all_conn = ""
                                    if not message_sent:
                                        sock.send(json.dumps({"status": "success", "list": package}).encode())
                                        message_sent = True
                                    break
                                time.sleep(1)
                                wait += 1
                            ...
                            sock.send(json.dumps({"status": "error", "message": "tunnel error"}).encode())
                    else:
                        sock.send(json.dumps({"status": "error", "message": "unknown args"}).encode())
                    ...
                    try:
                        ...
                        if str(a1) not in nat_banned_ips:
                            send_package(self._sock, 0, self.secret.encrypt(b'firewall;ban='+str(a1).encode('utf-8')))
                    ...
                    try:
                        if is_ip(a1) == True:
                            if "/" not in str(a1):
                                a1 = a1 + "/32"
                            send_package(self._sock, 0, self.secret.encrypt(b'firewall;priory='+str(a1).encode('utf-8')))
                    ...
                    try:
                        if a1 != "":
                            send_package(self._sock, 0, self.secret.encrypt(b'firewall;unban='+str(a1).encode('utf-8')))
                            ...
                            sock.send(json.dumps({"status": "error", "message": "unknown arguments"}).encode())
                    except:
                        sock.send(json.dumps({"status": "error", "message": "wrong argument"}).encode())
                    ...
                    try:
                        if a1 in ("tor", "vpn"):
                            if a2 == "yes" or a2 == "no":
                                send_package(self._sock, 0, self.secret.encrypt(b'edit_rule;'+str(a1).encode('utf-8')+b'='+str(a2).encode('utf-8')))
                                sock.send(json.dumps({"status": "success", f"message": f"value {a1} changed to {a2}"}).encode())
                        elif a1 == "rate":
                            if a2 in ("0", "1", "2", "3", "4", "5"):
                                send_package(self._sock, 0, self.secret.encrypt(b'firewall;rate='+a2.encode('utf-8')))
                    ...
                    sock.send(json.dumps({"status": "error", "message": "unknown command"}).encode())
            elif data["version"] == "mtunn_cch1":
                if data["command"]["execute"] == "forwarding":
                    sock.send(json.dumps({"remote": str(explore_domain)+":"+str(explore_port), "local": str(tunnel_two)}).encode())
            else:
                sock.send(b'x01x07')
        sock.close()
...
def main():
    ...
    parser.add_argument('--bufsize', metavar='<s>', help='changes the system buffer size to improve performance', type=int)
    parser.add_argument('--fastrun', metavar='<a>', help='launches a temporary TCP tunnel with short configuration', type=str)
    parser.add_argument('--config', metavar=' <f>', help='launches a configured tunnel from a configuration file', type=str)
    ...
    if args.bufsize:
        ...
        if is_android == False and platform.system().lower() in ("linux", "freebsd", "netbsd", "openbsd"):
            if int(args.bufsize) >= 32768 and int(args.bufsize) <= 1048576:
                if os.geteuid() == 0:
                ...
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] invalid buffer size")
                sys.exit(0)
        else:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;34mINFO\033[0m ] your system does not need to change the system buffer")
            sys.exit(0)
    if args.version:
        print(version+" "+build)
        sys.exit(0)
        ...
        def console_make_package(port, command):
            ...
            s.send(json.dumps(j).encode('utf-8'))
            if command != "stop":
                fragments = []
                while True:
                    chunk = s.recv(1024)
        ...
        if console_check_connection(port) != False:
            print("Welcome to console v1.3 (beta)")
        ...
        is_android: bool = hasattr(sys, 'getandroidapilevel')
        if is_android == False:
            if platform.system().lower() == "windows" and ctypes.windll.shell32.IsUserAnAdmin() != 1:
                print(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] superuser rights are required")
                sys.exit(0)
        path = mtunn_path()
        if path == None:
            ...
            sys.exit(8)
        if ipv() == False:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] failed to check ipv* connectivity")
            sys.exit(0)
        token = ""
        main_server = ""
        ee = ""
        try:
            ...
            if token == "" or main_server == "" or ee == "":
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] corrupted auth file")
                try: os.remove(path)
                except: pass
                sys.exit(0)
        if certs(main_server) == True:
            try:
                message = rpost(f"https://{main_server}:5569/auth/verify_token", json_data={"token": token, "email": ee}, timeout=10)["response"]["message"]
            except:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] failed to check account token")
                sys.exit(0)
            if message != "x03":
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] corrupted or wrong auth file, restart please")
                try: os.remove(path)
                except: pass
                sys.exit(0)
        else:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] failed to update server certificate")
            sys.exit(0)
        ...
        if index == 0:
            ...
            if _t == "" or _e == "":
                ...
                sys.exit(0)
            post = rpost(f"https://{main_server}:5569/auth/change_token", json_data={"token": _t, "email": _e}, timeout=10)["response"]
            if post["status"] == "success" and "token:" in post["message"]:
                with open(path, "w") as file:
                    file.write(post["message"].replace("token:", "")+"\n")
                    file.write(_e+"\n")
            ...
            main_server = data[2]
            del data
            post = rpost(f"https://{main_server}:5569/auth/quota_price", json_data={"token": token}, timeout=10)["response"]
            if post:
            ...
            if sure == "y" or sure == "Y" or sure == "Yes" or sure == "yes":
                post = rpost(f"https://{main_server}:5569/auth/replenish_balance", json_data={"type": "replenish"}, timeout=10)["response"]
                if post["status"] == "success":
        ...
        if is_android == False and platform.system().lower() in ("linux", "freebsd", "netbsd", "openbsd"):
            r, s = get_net()
            if r != 0 and r < 131072:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] your system has too small recv buffer, which may reduce the tunnel speed")
            if s != 0 and s < 65536:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] your system has too small send buffer, which may reduce the tunnel speed")
        ...
        if certs(main_server) == False:
            ...
            sys.exit(0)
        try:
            post = rpost(f"https://{main_server}:5569/auth/verify_token", json_data={"token": tt, "email": ee}, timeout=10)["response"]
            if post["message"] != "x03":
        ...
        whitelist = []
        try:
            whitelist.append(rget("https://ipinfo.io/?token=ecbdc84059119b", timeout=10)["response"]["country"])
        except:
            pass
        ...
        if int(os.cpu_count()) == 1:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] your processor only has 1 core which may have a high load")
        if str(lockis.version()) < "1.0.2":
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] too old version of «lockis»")
            sys.exit(7)
        try:
        ...
        try: int(cfg["firewall"]["protection"]["level"])
            ...
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] wrong or not enough arguments «level» in «protection»")
            sys.exit(7)
        try: cfg["firewall"]["services"]["vpn"]
        except:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] not enough arguments «vpn» in «services»")
            sys.exit(7)
        try: cfg["firewall"]["services"]["tor"]
        except:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] not enough arguments «tor» in «services»")
            sys.exit(7)
        try:
            if isinstance(cfg["firewall"]["blacklist"], list):
                blist = cfg["firewall"]["blacklist"]
            else:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] invalid arguments «blacklist» in «firewall», skipping list")
                blist = []
        except:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] wrong or not enough arguments «blacklist» in «firewall»")
            blist = []
        try:
            if isinstance(cfg["firewall"]["whitelist"], list):
                wlist = cfg["firewall"]["whitelist"]
            else:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] invalid arguments «whitelist» in «firewall», skipping list")
                wlist = []
        except:
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] wrong or not enough arguments «whitelist» in «firewall»")
            wlist = []
        try:
        ...
        try:
            if str(cfg["network"]["bandwidth"]) == "nolimit":
                ...
                if bandwidth < 1024:
                    print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] too small bandwidth, minimum 1 KB")
                    raise SystemExit
        ...
        try:
            if " " in str(cfg["network"]["socket"]["buffer"]):
                ...
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] invalid unit of measurement «buffer» in «socket»")
                raise SystemExit
            units = {
                "B": 1,
                "KB": 1024,
            ...
            if buffer < 131072:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] too small buffer, minimum 128 KB")
                raise SystemExit
            if buffer > 1048576:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] too big buffer, maximum 1 MB")
                raise SystemExit
        except (NameError, ValueError, KeyError):
            print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] failed to measure units «buffer» in «socket»")
            sys.exit(7)
        except SystemExit:
            sys.exit(7)
        if is_android == False and platform.system().lower() in ("linux", "freebsd", "netbsd", "openbsd"):
