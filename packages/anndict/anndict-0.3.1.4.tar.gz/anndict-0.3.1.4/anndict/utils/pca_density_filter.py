...
def pca_density_filter_adata(
    ...
    **kwargs
    ...
    data = adata.layer[use_layer] if use_layer in adata.layers else adata.X

    # Calculate density (skip if fewer than 10 cells)
    if data.shape[0] < 10:
        density, cutoff, variables_used_in_pca = np.ones(data.shape[0]), 0, np.zeros(data.shape[1], dtype=bool)
    else:
    ...
    index_vector = np.zeros(data.shape[0], dtype=bool)
    index_vector[high_density_indices] = True

    #record which cells were passed
    add_col_to_adata_obs(adata, np.arange(data.shape[0]), index_vector, new_filter_column_name)

    #record which variables were used in PCA
    adata.var[new_filter_column_name] = False
    adata.var.loc[variables_used_in_pca, new_filter_column_name] = True

    # Drop cells if requested
    if drop_cells:
        adata = adata[index_vector].copy()
        return adata
...
def pca_density_filter_main(
    ...
    - variables_used_in_pca: Indices of unique variables that were included in the PCA (empty list if all variables are constant).
    """
...
def pca_density_subsets(
    ...
    """
    warn(
        "pca_density_subsets() is deprecated and will be removed in a future version. Use pca_density_filter_adata() or pca_density_filter_main() instead.",
        DeprecationWarning,
        stacklevel=2
    )
    ...
    for label in unique_labels:
        ...
        if subset.shape[0] < 10:
            ...
            density, cutoff, _ = pca_density_filter_main(subset, n_components=3, threshold=0.10)
