...
class MultiAssetNewTradingBot:
   ...
   # 计算平均利润
    ...
    def check_position(self,symbol, position):
        ...
        take_profit_price = self.global_symbol_take_profit_price[symbol] 
        if position['side'] == 'long':
            if position['markPrice'] < take_profit_price:
                self.logger.debug(f"{symbol} 方向 {position['side']} - 市场价格 {position['markPrice']} 低于止止盈 {take_profit_price}，触发全局止盈")
                self.close_position(symbol, position)
                return
        elif position['side'] =='short':
            if position['markPrice'] > take_profit_price:
                self.logger.debug(f"{symbol} 方向 {position['side']} - 市场价格 {position['markPrice']} 高于止盈价 {take_profit_price}，触发全局止盈")
                self.close_position(symbol, position)
                return
        if self.global_symbol_stop_loss_flag.get(symbol, False):
            self.logger.debug(f"{symbol} - 全局止损已触发，跳过")
            return
