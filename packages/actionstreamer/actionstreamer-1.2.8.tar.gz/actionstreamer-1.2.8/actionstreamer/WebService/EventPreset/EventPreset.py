...
def create_event_preset(ws_config: WebServiceConfig, event_preset: Model.EventPreset, event_type: Model.EventType, event_parameters: Model.StartRecordingParameters | Model.StopRecordingParameters) -> tuple[int, str]:
    ...
    try:
        ...
        if event_type == Model.EventType.Video.Start_recording:
           event_parameters_json = Model.RecordingArgs (**event_parameters)
            ...
            event_parameters_json = Model.RTMPArgs(**event_parameters)

        elif event_type == Model.EventType.Video.Stop_RTMP:
            event_parameters_json = None

        elif event_type == Model.EventType.Video.Join_conference:
            event_parameters_json = Model.ConferenceArgs(**event_parameters)
