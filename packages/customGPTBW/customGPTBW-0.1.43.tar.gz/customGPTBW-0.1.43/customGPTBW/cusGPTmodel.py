...
class ModelGPT():
    ...
    def train_model(self, dataset_,val_dataset=None ,epochs=1, lr=5e-5, final_lr=5e-5, loss_avg=1500, change_batch=500, split_training=False, val_split=0.1,val_log=500,replicas=8):
        ...
        for batch in dataset_.take(1):
            global_batch_size = batch[0]['input_ids'].shape[0]
        
        if self.using_ == "TPU":
            global_batch_size = replicas * global_batch_size
        ...
        def val_func(strategy,dist_val_dataset):
            ...
            for batch in dist_val_dataset:
                ...
                loss = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses, axis=None)  # Change to SUM
