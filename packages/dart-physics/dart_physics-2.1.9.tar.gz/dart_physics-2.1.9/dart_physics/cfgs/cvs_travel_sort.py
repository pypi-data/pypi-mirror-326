...
def random_xyz(i):
    return [random.uniform(-0.2, 0.2), random.uniform(0.2, 0.35), 0.05 + 0.03 * i ]
...
def reset_function(model, data, robot_cfg, task_cfg): 
    ...
    for ii, obj in enumerate(task_cfg['objects']): 
        ...
        if "basket" in obj: 
            ...
            new_pos[0:2] += np.random.uniform(-0.01, 0.01, 2)
            new_quat = R.from_euler('xyz', [90, 0, 0], degrees = True).as_quat(scalar_first = True).tolist()
...
task_cfg = { 
    ...
    "objects": ["wood_basket", "wood_basket_2", "black_toothpaste_basket"] + utensils, 

    "task_description": "Sort items on the table; same items should be in the same basket.",
