...
class YfSDK:
    ...
    async def balance_sheet(
        ...
        await self.db.batch_insert_dataframe(df, table_name='balance_sheet', unique_columns='ticker')
    ...
    async def get_cash_flow(
        ...
        await self.db.batch_insert_dataframe(df, table_name='cash_flow', unique_columns='ticker')
    ...
    async def get_all_candles(self, tickers: str):
        ...
        try:
            ...
            df['ticker'] = tickers
            # Uncomment the following line if you wish to insert the data into a database table.
            # await self.db.batch_insert_dataframe(df, table_name='candles', unique_columns='ticker')
            return chart_data
    ...
    def fast_info(self, ticker: str):
        """
        Returns fast info for a ticker.

        Arguments:
            ticker (str): The ticker symbol.

        Returns:
            A formatted DataFrame containing fast info.
        """
        data = yf.Ticker(ticker).get_fast_info().items()
