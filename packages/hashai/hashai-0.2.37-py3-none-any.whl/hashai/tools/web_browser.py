...
def get_system_prompt(user_input: str,
                      ...
                      tabs: str = "None") -> str:
    """
    Generate a dynamic system prompt that instructs the LLM to produce a fully structured JSON plan.
    The plan should include an optional "ui_index" field for actions like click and type.
    
    Parameters:
      - user_input: The high-level instruction from the user.
      - current_url: The URL of the current page.
      - current_date: The current date/time string.
      - interactive_elements: A string representation of interactive elements (if available).
      - tabs: A string representing the list of open tabs.
...
2. For actions like "click" and "type", include an optional field "ui_index" that indicates the number (from the UI labeling step) of the UI element to be used.
3. Always include a "label_ui" action immediately after navigation to map all interactive elements on the page.
4. Use the visual context (the overlaid numbered boxes) to verify element locations.
5. Sequence the actions logically.
6. End the plan with a "done" action if the task is complete.
...
{{
  ...
  "action": [
    {{
      "action_name": {{
         // action-specific parameters; for actions like "click" or "type", include "selector", "value", and optionally "ui_index"
      }}
    }},
...
- label_ui: Scan and overlay the page with colored boxes and numbered labels for interactive elements.
- click: Click on an element (optionally specify "ui_index").
- type: Type text into an input field (optionally specify "ui_index").
- wait: Wait for a specified number of seconds.
- wait_for_ajax: Wait until AJAX/network activity has subsided.
- scroll: Scroll to a specified element or to the bottom of the page.
...
- drag_and_drop: Simulate a drag-and-drop operation.
...
class WebBrowserTool(BaseTool):
    ...
    synonyms_cache: Dict[str, List[str]] = {}  # Dynamic synonyms cache
    ...
    def execute(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Workflow:
         1. Use the LLM to generate a JSON plan from the user's instruction. The plan includes a "ui_index"
            for actions like click and type.
         2. Automatically insert a UI-labeling step (label_ui) immediately after the first navigation task.
         3. Process tasks sequentially. If a task fails, retry it (up to the overall maximum).
         4. When a task indicates a ui_index, log that number (from the UI-labeling mapping).
         5. Report overall success only if every task completes.
        """
        driver = None
        ...
        try:
            ...
            # Generate a dynamic plan using the LLM with our system prompt.
            # (For simplicity, interactive elements and tabs are not provided here.)
            ...
            while task_index < len(plan.tasks) and overall_attempts < self.max_overall_attempts:
                ...
                # If the task includes a ui_index, log it.
                if "ui_index" in task:
    ...
    def _generate_plan(self, query: str, current_url: str) -> BrowserPlan:
        ...
        This method calls get_system_prompt to obtain a prompt with full context and expected JSON format.
        """
        current_date = datetime.now().strftime("%Y-%m-%d %H:%M")
        # In a more advanced scenario, interactive_elements and tabs could be determined dynamically.
    ...
    def _parse_plan(self, response: str) -> BrowserPlan:
        """Parse the JSON plan from the LLM response robustly."""
        try:
    ...
    def _execute_safe_task(self, driver: webdriver.Chrome, task: Dict[str, Any],
                             handler: Callable[[webdriver.Chrome, Dict[str, Any]], Dict[str, Any]]) -> Dict[str, Any]:
        """Safely execute a task and catch any errors."""
        try:
    ...
    def _advanced_find_element(self, driver: webdriver.Chrome, keyword: str) -> Optional[webdriver.remote.webelement.WebElement]:
        """
        Use advanced fallback to locate an element via fuzzy matching of attributes and inner text,
        and log the UI label number if it matches an entry in the UI mapping.
        """
    ...
    def _handle_navigation(self, driver: webdriver.Chrome, url: str) -> Dict[str, Any]:
        """Navigate to a URL (adding 'https://' if necessary)."""
    ...
    def _handle_click(self, driver: webdriver.Chrome, selector: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If a "ui_index" is provided in the task, log that index.
        """
        if "ui_index" in task:
    ...
    def _handle_typing(self, driver: webdriver.Chrome, selector: str, text: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If a "ui_index" is provided in the task, log that index.
        Use advanced fallback and recovery strategies if necessary.
        """
        if "ui_index" in task:
            logger.info(f"Task indicates to use UI element with index {task['ui_index']}.")
        try:
        ...
        try:
            ...
            if "invalid element state" in error_message.lower():
                logger.warning("Invalid element state detected. Attempting to remove 'readonly' attribute and use JS injection.")
                try:
    ...
    def _handle_hover(self, driver: webdriver.Chrome, selector: str) -> Dict[str, Any]:
        """Hover the mouse over the element specified by the selector."""
        try:
            element = WebDriverWait(driver, self.default_timeout).until(
    ...
    def _handle_switch_tab(self, driver: webdriver.Chrome, value: str) -> Dict[str, Any]:
        """
        Switch to a different tab. 'value' may be an index (as a string) or the keyword 'new'.
        """
        try:
    ...
    def _handle_label_ui(self, driver: webdriver.Chrome, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        on interactive elements. Different tags are given different border colors. The overlaid number
        (the UI label) is returned as part of a JSON mapping.
        """
        try:
            script = """
            (function() {
