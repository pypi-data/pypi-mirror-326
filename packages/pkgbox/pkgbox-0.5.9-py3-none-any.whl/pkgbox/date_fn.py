...
class Cdt:
    ...
    def date_split_extended(start_date, end_date, interval_type, dt_fmt="%Y-%m-%d"):
    """
    ...
    Parameters:
      ...
      interval_type (str): One of "week", "month", "quarter", "year".
      dt_fmt (str): Date format string.
    Returns:
      List of tuples of strings [(interval_start, interval_end), ...]."""
    # Convert input strings to date objects
    start = datetime.datetime.strptime(start_date, dt_fmt).date()
    end   = datetime.datetime.strptime(end_date, dt_fmt).date()
    intervals = []
    current = start

    while current <= end:
        if interval_type.lower() == 'week':
            # Define a week as Monday to Sunday.
            # current.weekday() returns 0 (Monday) to 6 (Sunday).
            # So the end of the week is:
            days_to_sunday = 6 - current.weekday()
            period_end = current + datetime.timedelta(days=days_to_sunday)
            ...
            if current.month <= 3:
                period_end = datetime.date(current.year, 3, 31)
            elif current.month <= 6:
                period_end = datetime.date(current.year, 6, 30)
            elif current.month <= 9:
                period_end = datetime.date(current.year, 9, 30)
            else:
                period_end = datetime.date(current.year, 12, 31)
        elif interval_type.lower() == 'year':
            period_end = datetime.date(current.year, 12, 31)
        else:
            raise ValueError("Unsupported interval type: {}".format(interval_type))
        ...
        # Set the start of the next interval to be the day after period_end
        current = period_end + datetime.timedelta(days=1)
