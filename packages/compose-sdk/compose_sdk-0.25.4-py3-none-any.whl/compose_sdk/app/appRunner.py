...
class AppRunner:
    ...
    async def render_ui(
        ...
        *,
        ...
        try:
            ...
            if self.debug:
                time_taken = time.time() - most_recent_time
                log_debug(
                    f"Page add{f' ({key})' if key else ''}",
                    f"Generated static layout in {time_taken * 1000} ms",
                )
            ...
            if self.debug:
                ...
                log_debug(
                    f"Page add{f' ({key})' if key else ''}",
                    f"Compressed static layout in {time_taken * 1000} ms",
                )
    ...
    async def on_state_update(self):
        ...
        for renderId in self.renders:
            if self.debug:
                log_debug("Page update", f"checking section: {renderId}")
            ...
            if self.debug:
                ...
                log_debug(
                    "Page update",
                    f"generated new layout for {renderId} in {time_taken * 1000} ms",
                )
                most_recent_time = time.time()
            ...
            if self.debug:
                ...
                log_debug(
                    "Page update",
                    f"validated new layout for {renderId} in {time_taken * 1000} ms",
                )
                most_recent_time = time.time()
            ...
            if self.debug:
                ...
                log_debug(
                    "Page update",
                    f"generated diff for {renderId} in {time_taken * 1000} ms",
                )
        ...
        if self.debug:
            ...
            log_debug(
                "Page update", f"computed page diff in {algorithm_time * 1000} ms"
            )
        ...
        for table in tables:
            if (
                ...
                await self.api.send(
                    {
                        "type": EventType.SdkToServer.STALE_STATE_UPDATE,
                        "executionId": self.executionId,
    ...
    async def on_table_page_change_hook(
        ...
        try:
            ...
            if table_state["stale"] != Stale.FALSE:
                old_stringified = JSON.stringify(
                    {
                ...
                new_stringified = JSON.stringify(
                    {
                        "offset": offset,
                        "search_query": search_query,
                ...
                if old_stringified == new_stringified:
                    await self.api.send(
                        {
                            "type": EventType.SdkToServer.STALE_STATE_UPDATE,
                            "executionId": self.executionId,
            ...
            await self.api.send(
                {
                    "type": EventType.SdkToServer.TABLE_PAGE_CHANGE_RESPONSE,
                    "executionId": self.executionId,
                    "renderId": render_id,
                    "componentId": component_id,
