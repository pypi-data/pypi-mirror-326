...
from .core import EventType, log_debug
...
class ComposeClient:
    ...
    async def handle_browser_event(self, event: Dict) -> None:
        if event["type"] == EventType.ServerToSdk.START_EXECUTION:
            if self.debug:
                log_debug("Browser", f"Start app execution for {event['appRoute']}")
        ...
        if event["type"] == EventType.ServerToSdk.ON_CLICK_HOOK:
            ...
            if self.debug:
                log_debug("Browser", f"input event for {event['componentId']}")
            ...
            if self.debug:
                log_debug("Browser", f"table row action for {event['componentId']}")
            ...
            if self.debug:
                log_debug(
                    "Browser", f"page confirmation response for {event['componentId']}"
                )
            ...
            if self.debug:
                log_debug("Browser", f"modal closed for {event['renderId']}")

            runner.on_close_modal(event["renderId"])

        elif event["type"] == EventType.ServerToSdk.ON_TABLE_PAGE_CHANGE_HOOK:
            if self.debug:
                log_debug("Browser", f"table page change for {event['componentId']}")
